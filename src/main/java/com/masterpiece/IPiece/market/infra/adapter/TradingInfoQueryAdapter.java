...
import com.masterpiece.IPiece.market.infra.jpa.DisclosureJpaRepository;
import com.masterpiece.IPiece.market.infra.jpa.ProductTradingInfoJpaRepository;
...
public class TradingInfoQueryAdapter implements TradingInfoQueryPort {
    ...
    private final ProductTradingInfoJpaRepository tradingInfoRepo;
    private final DisclosureJpaRepository disclosureRepo;

    @Override
    public Optional<ProductTradingInfo> findTradingInfo(Long productId) {
        return tradingInfoRepo.findById(productId);
    }

    @Override
    public List<Disclosure> findDisclosures(Long productId, int limit) {
        return disclosureRepo.findByProductId(
                productId,
