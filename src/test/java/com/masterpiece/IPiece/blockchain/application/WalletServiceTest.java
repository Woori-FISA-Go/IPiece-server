...
import com.masterpiece.IPiece.integration.besu.BesuClient; // BesuClient import
...
class WalletServiceTest {
    ...
    void mintKrwt_Success() {
        ...
        // ✅ besuClient.mintKrwt 호출 시 가짜 txHash 반환하도록 Mocking
        when(besuClient.mintKrwt(any(String.class), any(BigInteger.class))).thenReturn("0xmockTxHashMint");
    ...
    void burnKrwt_Success() {
        ...
        when(virtualAccountRepository.findByUser_UserId(userId)).thenReturn(Optional.of(mockVirtualAccount));
        when(virtualAccountRepository.save(any(VirtualAccount.class))).thenReturn(mockVirtualAccount);
        // ✅ besuClient.burnKrwt 호출 시 가짜 txHash 반환하도록 Mocking
        when(besuClient.burnKrwt(any(String.class), any(BigInteger.class))).thenReturn("0xmockTxHashBurn");
        when(krwtOperationRepository.save(any(KrwtOperation.class))).thenAnswer(invocation -> {
            KrwtOperation op = invocation.getArgument(0);
